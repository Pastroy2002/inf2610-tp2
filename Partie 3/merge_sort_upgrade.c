...
int main(int argc, char *argv[]) {
    ...
    log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, "Initial array = [");
    for (int i = 0; i < array_size; i++) {
        log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, "%d", shared_data->array[i]);
        if (i < array_size - 1) log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, ", ");
    }
    ...
    printf("Temps d'exécution du tri sans journalisation : %.6f secondes\n", elapsed_time);

   
    log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, "Final sorted array = [");
    for (int i = 0; i < array_size; i++) {
        log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, "%d", shared_data->array[i]);
        if (i < array_size - 1) log_index += snprintf(log_buffer + log_index, LOG_BUFFER_SIZE - log_index, ", ");
    }
